...
// ==================== Dock 拖拽排序功能 ====================
    ...
    function scheduleWeatherSuggestionSearch(rawValue) {
      ...
      weatherSearchTimer = setTimeout(async () => {
        try { weatherSearchSuggestions = await qWeatherSearchLocations(value); }
        catch (err) { console.warn('[Bingyu Weather] city suggestion failed:', err && err.message ? err.message : err); weatherSearchSuggestions = []; }
        finally { weatherSearchLoading = false; updateWeatherSuggestionBox(); }
      }, 260);
    }
    ...
    function applyWeatherListEntry(item, entry) {
      ...
      if (entry.weatherData) {
        ...
        item._weather = data;
        item.weatherCache = { updatedAt: Date.now(), provider: data.source || 'QWeather', data };
        item._weatherError = '';
        saveDesktopState();
